...
use super::playerctl::{load_album_art, run_playerctl, SendPixbuf};
...
pub fn start_player_polling_loop(
    ...
    let (art_sender, mut art_receiver) = tokio::sync::mpsc::unbounded_channel::<(String, String, Option<(SendPixbuf, SendPixbuf)>)>();
    ...
    glib::MainContext::default().spawn_local(async move {
        while let Some((url, app_icon_name, result)) = art_receiver.recv().await {
            if url == *last_attempted_url_clone.borrow() {
                if let Some((SendPixbuf(s_pb), SendPixbuf(l_pb))) = result {
...
fn update_player_view(
    ...
    player_name_raw: &str,
    art_url: &str,
    art_sender: &tokio::sync::mpsc::UnboundedSender<(String, String, Option<(SendPixbuf, SendPixbuf)>)>,
) {
