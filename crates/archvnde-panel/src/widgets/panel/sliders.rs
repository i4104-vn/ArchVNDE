...
fn get_default_device(is_source: bool) -> String {
    if let Ok(output) = std::process::Command::new("wpctl")
        .args(&["inspect", if is_source { "@DEFAULT_AUDIO_SOURCE@" } else { "@DEFAULT_AUDIO_SINK@" }])
        .output()
    {
        let stdout = String::from_utf8_lossy(&output.stdout);
        for line in stdout.lines() {
            if line.contains("node.name =") {
                if let Some(pos) = line.find('"') {
                    let end = line[pos+1..].find('"').unwrap_or(0);
                    return line[pos+1..pos+1+end].to_string();
                }
            }
        }
    }
    ...
    {
        let stdout = String::from_utf8_lossy(&output.stdout);
        let prefix = if is_source { "Default Source: " } else { "Default Sink: " };
        for line in stdout.lines() {
...
fn get_audio_devices(is_source: bool) -> Vec<AudioDevice> {
    ...
    let cmd = if is_source { "sources" } else { "sinks" };
    ...
    if let Ok(out) = output {
        ...
        for line in stdout.lines() {
            ...
            if !current_name.is_empty() && !current_desc.is_empty() {
                let is_default = current_name == default_name;
...
fn populate_audio_menu(popover: &gtk4::Popover, update_mute_btn: Rc<dyn Fn()>) {
    ...
    if sinks.is_empty() {
        ...
        for sink in sinks {
            ...
            let icon = archvnde_common::icon::get_icon_colored(
                "volume", 14, 
                if sink.is_default { "#ffffff" } else { "rgba(255, 255, 255, 0.6)" }
            );
    ...
    for source in sources {
        ...
        let icon = archvnde_common::icon::get_icon_colored(
            "microphone", 14, 
            if source.is_default { "#ffffff" } else { "rgba(255, 255, 255, 0.6)" }
        );
